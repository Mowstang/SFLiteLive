<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Claims.aspx.cs" Inherits="SFLite.Claims" %>
...
<script type="text/javascript">
  ...
  ;
        ...
        var element_to_scroll_to = document.getElementById('starthere');
...
function goViewDocs(params) {
    window.location = 'ViewDocs.aspx?' + params;
}
function goRooms(params) {
    window.location = 'PickRooms.aspx?' + params;
}
function goEdit(params) {
    window.location = 'EditHours.aspx?' + params;
}
function goPR(params) {
    window.location = 'PRFormNew.aspx?' + params;
}

setTimeout('window.location.href = window.location.href', 600000);
...
<div id="ClaimList" runat="server">
    <h4 class="ui center aligned header"><%= userName %></h4>
    <div class="col-sm-4">
        <% getActiveClaims(); %>
    </div>
</div>
